...
interface UseOrderReturn {
  ...
  cancelOrder: (id: string, customerId: string, approve: boolean) => Promise<Order | null>;
...
export function useOrder(): UseOrderReturn {
  ...
  const checkout = useCallback(async (request: OrderRequest): Promise<OrderResponseDTO | null> => {
    setLoading(true);
    setError(null);
    try {
  ...
  const calculatePrice = useCallback(async (cart: Cart): Promise<number | null> => {
    setLoading(true);
    setError(null);
    try {
  ...
  const getCartSummary = useCallback(async (cart: Cart): Promise<CartSummary | null> => {
    setLoading(true);
    setError(null);
    try {
  ...
  const cancelOrder = useCallback(async (id: string, customerId: string, approve: boolean): Promise<Order | null> => {
    setLoading(true);
    setError(null);
    try {
  ...
  const getCustomerIdByUsername = useCallback(async (username: string): Promise<string | null> => {
    setLoading(true);
    setError(null);
    try {
  ...
  const getRecentOrders = useCallback(async (limit: number = 10): Promise<Order[] | null> => {
    setLoading(true);
    setError(null);
    try {
