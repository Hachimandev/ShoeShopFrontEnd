...
import { ShoppingBag, ChevronLeft, Star, Truck, ShieldCheck, Heart, Share2 } from "lucide-react";
...
export default function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const uniqueSizes = Array.from(new Set(product.productDetails?.map(d => d.size))).sort();
  // Lấy danh sách màu sắc duy nhất
  const uniqueColors = Array.from(new Set(product.productDetails?.map(d => d.color)));
          ...
          <div className="flex flex-col space-y-10">
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-slate-400 capitalize">{product.reviews || 0} verified reviews</span>
              </div>
              <h1 className="text-5xl font-black tracking-tighter leading-[1.1]">{product.productName || product.name}</h1>
              <div className="flex items-baseline gap-4">
                <p className="text-4xl font-black text-primary">${product.price?.toLocaleString()}.00</p>
                {product.originalPrice && (
                  <p className="text-xl text-slate-400 line-through font-bold">${product.originalPrice?.toLocaleString()}.00</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-6">
                  ...
                  {uniqueSizes.map((size) => (
                    <button
                      ...
                      className={`h-14 border-2 rounded-2xl flex items-center justify-center text-lg font-black transition-all active:scale-95 ${
                        selectedSize === size ? "border-primary text-primary ring-2 ring-primary" : "border-slate-100 hover:border-slate-300"
                      }`}
                    >
                      {size}
                    </button>
              ...
              <div className="space-y-3">
                <h3 className="font-black text-sm uppercase tracking-widest">Color: <span className="text-slate-500 font-bold uppercase ml-1">{selectedColor}</span></h3>
                  ...
                  {uniqueColors.map((color) => (
                    <button
                      ...
                      className={`h-10 px-4 rounded-full border-2 transition-all font-bold text-sm ${
                        selectedColor === color ? "border-primary bg-primary text-white" : "border-slate-200 hover:border-slate-300 text-slate-600"
                      }`}
                    >
                      {color}
                    </button>
            ...
            <div className="flex flex-col gap-4">
              <Button size="lg" className="h-20 text-xl font-black rounded-3xl shadow-xl shadow-primary/20 hover:scale-[1.02] transition-transform">
                ADD TO BAG — ${product.price?.toLocaleString()}.00
              </Button>
            ...
            <div className="grid grid-cols-1 gap-4 pt-6">
              ...
              <div className="flex items-center gap-4 p-5 rounded-3xl bg-slate-50 border border-slate-100">
                ...
                <div>
                  <p className="text-sm font-black uppercase tracking-tight">Authenticity Guaranteed</p>
                  <p className="text-xs font-bold text-muted-foreground">All products are 100% genuine and verified</p>
                </div>
              </div>
            </div>
